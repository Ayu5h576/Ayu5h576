...
BASE_DIR = Path(r"C:\Users\ayush\.gemini\antigravity\scratch\github-profile-readme")
DATA_DIR = BASE_DIR / "readme-data"
...
# 2. focus.md
focus_content = """<!-- CURRENT FOCUS — Edit this file to update the Current Focus section in your README -->
...
with open(DATA_DIR / "focus.md", "w", encoding="utf-8") as f:
...
quotes = [
  ...
  { "text": "One of the best things about programming is that we can iterate rapidly, learn from mistakes, and improve.", "author": "Unknown" },
  ...
  { "text": "In software engineering, what can go wrong, will go wrong.", "author": "Murphy's Law" },
  { "text": "You can't improve what you don't measure.", "author": "Peter Drucker" },
  { "text": "Build things that matter.", "author": "Ayush Rawat" },
  { "text": "AI is not a replacement for human creativity; it's a tool that amplifies it.", "author": "Unknown" },
  { "text": "The best products solve a problem users didn't know they had.", "author": "Unknown" },
  ...
  { "text": "Move fast and build things that last.", "author": "Unknown" },
  ...
  { "text": "Shipping beats perfection. Perfection beats nothing.", "author": "Unknown" }
]
...
tips = [
  ...
  { "category": "Debugging", "tip": "When stuck, explain the bug out loud. Articulating the problem often reveals the solution." },
  ...
  { "category": "Testing", "tip": "Test behavior, not implementation details. Fragile tests slow down refactoring." },
  { "category": "Productivity", "tip": "Focus in deep blocks (e.g. Pomodoro 25/5). Rest cycles maintain high cognitive performance." },
  { "category": "Open Source", "tip": "Read the source code when documentation is sparse. The codebase is the ultimate ground truth." },
  { "category": "Database", "tip": "Always index columns used in WHERE, JOIN, or ORDER BY clauses to prevent slow full table scans." },
  { "category": "Node.js", "tip": "Never block the event loop. Offload heavy computations to worker threads." },
  { "category": "React State", "tip": "Lift state only as high as necessary. Keep transient state local to the component." },
  { "category": "DevOps", "tip": "Automate deployment pipelines early. Manual steps introduce human error." },
  { "category": "Code Review", "tip": "Reviews are for learning and quality, not gatekeeping. Provide constructive suggestions." },
  { "category": "Architecture", "tip": "Enforce separation of concerns. Data fetching, transformation, and rendering belong in distinct modules." }
]

with open(DATA_DIR / "tips.json", "w", encoding="utf-8") as f:
...
def generate_activity_section() -> str:
    ...
    if not events:
        return "- Updated project documentation and repositories."
...
with open(SCRIPTS_DIR / "update_readme.py", "w", encoding="utf-8") as f:
    f.write(update_script_content)

# 6. workflow file
workflow_content = """name: Update Profile README
...
with open(WORKFLOWS_DIR / "update-readme.yml", "w", encoding="utf-8") as f:
    f.write(workflow_content)

# 7. Sleek, minimal, high-end README.md
...
print("All clean files created successfully with UTF-8 encoding!")
